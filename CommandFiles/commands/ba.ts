...
export const meta: CommandMeta = {
  ...
  version: "1.0.0",
  description: "Generate BA-style images using Wildan Suldyir API",
  category: "NSFW",
...
export const langs = {
  en: {
    ...
    generating: "🔥 | Generating BA image, please wait...",
...
export const entry = defineEntry(
  async ({ input, output, args, langParser }) => {
    ...
    try {
      ...
      const filePath = path.join(CACHE_DIR, `ba_${Date.now()}.png`);
      fs.writeFileSync(filePath, response.data);

      await output.unsend(waitMsg.messageID);

      await output.reply({
        body: `✅ BA image generated successfully!\n🧠 Prompt: ${prompt}`,
        attachment: fs.createReadStream(filePath),
      });

      fs.unlinkSync(filePath);
    } catch (err: any) {
      console.error("BA AI Command Error:", err.message || err);
      await output.unsend(waitMsg.messageID);
